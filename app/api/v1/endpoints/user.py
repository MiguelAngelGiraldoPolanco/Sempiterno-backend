...
def obtener_usuario_por_id(id_user: int, db: Session = Depends(database.get_session)):
    return user_service.obtener_usuario_por_id(db, id_user)


@router.get("/email/{email_user}", response_model=user.UserRead)
def obtener_usuario_por_emial(
    email_user: EmailStr, db: Session = Depends(database.get_session)
):
...
def eliminar_usuario(id_user: int, db: Session = Depends(database.get_session)):
